...
{	
	...
	if ((w && OFPFW_IN_PORT) || (m1->OFB_IN_PORT == m2->OFB_IN_PORT)) { 
	    if (m1->OFB_IP_PROTO == 6 || m1->OFB_IP_PROTO == 17) {	
       ...
       //    			&& ((w & OFPFW_DL_DST) || (!m1->OFB_ETH_DST.compareTo(m2->OFB_ETH_DST))) );
		//}
		//IP Packet
		//general rule for matching MAC fields
		//if (m1->OFB_ETH_TYPE == 2048) {
		//   return (((w & OFPFW_DL_TYPE) || (m1->OFB_ETH_TYPE == m2->OFB_ETH_TYPE ))
		//       		&& ((w & OFPFW_DL_SRC) || (!m1->OFB_ETH_SRC.compareTo(m2->OFB_ETH_SRC)))
		//     			&& ((w & OFPFW_DL_DST) || (!m1->OFB_ETH_DST.compareTo(m2->OFB_ETH_DST))));
...
bool Flow_Table::addEntry(oxm_basic_match *match, entry_data* data) {
    pair<multimap<oxm_basic_match, entry_data>::iterator, bool> ret;
    
    multimap<oxm_basic_match, entry_data>::iterator it;
    if (entry_map.size() == size) { //flow table is full, then do not insert the new entry and infrom controller
        //send error message to the controller through OFA_switch
        return false;
    }
    else { // add the new entry
        for (it = entry_map.begin(); it!=entry_map.end(); it++) {
        	if ((it->second.prior->priority) < (data->prior->priority)) {
        		entry_map.insert(it, pair<oxm_basic_match, entry_data>(*match, *data));
        		break;
        	}
        }
        if(it == entry_map.end())
...
bool Flow_Table::deleteEntry(oxm_basic_match *match)
{
    ...
    if (iter == entry_map.begin() && flow_fields_match(match, const_cast<oxm_basic_match*> (&iter->first), iter->first.wildcards)) {
        entry_map.erase(iter);
        delete match; 
    }
...
void Flow_Table::printFlowTable() {
	...
	std::cout<<"===================================================================" << endl;
	std::cout << " [" << simTime() << "]" << " [" << getParentModule()->getFullPath() <<" ] Flow Table:" << endl;
	std::cout<<"Table size = " << entry_map.size() << endl;
